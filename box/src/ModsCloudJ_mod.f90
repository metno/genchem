...
! Read in fast-J X-sections (spectral data)
      call RD_XXX(JXUNIT,trim(cloudj_indir)//'FJX_spec.dat')

! Read in cloud scattering data
      call RD_CLD(JXUNIT,trim(cloudj_indir)//'FJX_scat-cld.dat')

! Read in aerosols scattering data
      call RD_MIE(JXUNIT,trim(cloudj_indir)//'FJX_scat-aer.dat')

! Read in UMich aerosol scattering data
      call RD_UM (JXUNIT,trim(cloudj_indir)//'FJX_scat-UMa.dat')

! Read in T & O3 climatology used to fill e.g. upper layers or if O3 not calc.
      call RD_PROF(JXUNIT,trim(cloudj_indir)//'atmos_std.dat')
...
!---CTM call:  read in J-values names and link to fast-JX names
      call RD_JS_JX(JXUNIT,trim(cloudj_indir)//'FJX_j2j.dat', TITLEJXX,NJXX)
...
!---EmChem family of chemistry scheme; drop all other xsects to save CPU time
      if (CM_schemes_ChemRates(:7) .eq. " EmChem") then 
        write(6,'(a)')  &
