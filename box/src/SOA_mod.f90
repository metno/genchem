module OrganicAerosol_mod
  ...
  !--------------------------------------------------------------------------
   ...
   use ChemGroups_mod         ! eg.  OM25
   use Config_module,        only: MasterProc !A18
   use Debug_module,         only: debug   ! debug%SOA, integer
   use MetFunctions,         only: StandardAtmos_kPa_2_km ! for  Hz scaling
   use ModelConstants,       only: CHEMTMIN, CHEMTMAX
                                    !D16 K2 => KMAX_MID, K1 => KCHEMTOP
   use PhysicalConstants_mod,only: AVOG, RGAS_J 
   use SmallUtils_mod,       only: find_index
   use ZchemData_mod,        only: itemp, xn => xChem, Fgas, Fpart
   implicit none
   private
   ...
   ! We store OM values in between calls (in 3-D fields in EMEP), to allow
   ! the next G/P partitioning calculation to  start off with values of COA,
   ! mw and Fgas which are about right. Ensures that very few iterations are
   ! needed.
  ...
  real, parameter, public :: SMALLFN  = 1.0e-20 ! Minimum value of ug allowed
   ...
   ! Deduce k-extent from xn array, store here:
     integer, private, save :: nzlevels

   contains
   !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   subroutine Init_OrganicAerosol(zmid,first_tstep)
   real, dimension(:), intent(in) :: zmid ! extent of vertical layers  (eg 2,20 for emep)
   logical, intent(in) :: first_tstep   ! same as first_call for box model
   integer :: ispec, is,  it, k
  ...
  ! Indices of Cstar and DeltaH groups in CM_ChemGroups
   ...
   if ( first_call ) then ! =========================================
      ...
      itot_om25 = find_index( 'OM25_p',    species(:)%name, any_case=.true. )  !NOTE CASE!
     ...
     !D16 Change system. CAREFUL - old system could have K1,K2=2,20 (emep!)
     !======================
      nzlevels = size(xn,dim=2)
     !======================
      allocate(COA(nzlevels)) !D16 (K1:K2))
      allocate(BGND_OC(nzlevels)) !D16 (K1:K2))
      allocate(BGND_OA(nzlevels)) !D16 (K1:K2))
      allocate(ug_semivol(S1:S2,nzlevels)) !D16 K1:K2)) ! semivol consequetive
      allocate(ug_nonvol(NUM_NVABSOM,nzlevels)) !D16 K1:K2))!Not consequetive!
     !D16 allocate(h_km(K2) )
     ...
     ! QUERY Here.
        ...
        !DSSOA p_kPa(:) = 0.001*( A_mid(:)+B_mid(:)*101325.0 ) ! Pressure in kPa
        !DSSOA h_km     = StandardAtmos_kPa_2_km(p_kPa)
   
      !DS h_km     = StandardAtmos_kPa_2_km(0.001*StandardP_Pa)
      BGND_OC(:)= 0.2 * 1.005 ! ng/m3 !!! will give 0.2 ugC/m3 at z=0 m
   
      if(dbg ) write(*,"(a,a4,2a8)") "BGND_OC ", "k", "zmid", "BGND_OC"
      do k = 1, nzlevels !D16 K1, K2
         !D16 BGND_OC(k) = BGND_OC(k) * exp( -h_km(k)/9.1 )
         BGND_OC(k) = BGND_OC(k) * exp( -0.001*zmid(k)/9.1 )
         if(dbg ) write(*,"(a,i4,f8.1,f8.3)") "BGND_OC ", k, zmid(k), BGND_OC(k)
      end do
     ...
     ! Set up Tables for Fcond 
      ...
      do ispec=S1,S2
         ...
         if ( dbg ) then ! Checks that GenChem worked ok
            write(*,"(a,2i6,2f10.3,1x,2i4,a)") "CSTAR ",  is, ispec, &
              chemgroups_factors(igrp_Cstar)%factors(is), &
              chemgroups_factors(igrp_DeltaH)%factors(is), &
              chemgroups_factors(igrp_Cstar)%species(is),  &
              chemgroups_factors(igrp_DeltaH)%species(is), &
       ...
       if ( dbg ) then 
         do is = S1, S2
            write(6,"(a,i4,1x,a20,2f7.1,8es10.2)") &
...
! preceding call to OrganicAerosol for a different set of i,j.
  ...
  do iter = 1, NITER
     ...
     ! ====================================================================
      ...
      if( dbg1 ) then
         ...
         nz= 1 ! nzlevels debug for 1=surface for ESX (top for EMEP!)
 ...
 subroutine Reset_OrganicAerosol(i_pos,j_pos,debug_flag)
   ...
   nz = 1 ! 1=surface nzlevels  ! just for brevity
      ...
      if(  dbg ) then
        do k = 1, nz  !K1, K2
           write(*,"(a,3i4,1x,a15,9es12.3)") "OFSOA fac ", n, itot, k, &
             ...
             xn(itot_om25,k) * molcc2ugm3 !BOX,  Grid_COA(i_pos,j_pos,k)
         end do
      end if
   end do ! n
   !BOXSOAGrid_COA(i_pos,j_pos,:) = xn(itot_om25,:) * molcc2ugm3
   first_call = .false.
...
!TSTEMX use ZchemData_mod,  only :  Alloc1Dchem, itemp !, xn => xChem, Fgas, Fpart
