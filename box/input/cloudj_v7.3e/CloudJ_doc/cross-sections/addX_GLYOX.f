...
c XNEW = cross section (cm2) as a function of WW.
      ...
      real*8  :: FB(10000), WB(10000), INPWW(10), QY(10000), WBQ(10000)
      real*8  :: IND(10), INDQ(10)
      integer :: NGLYOX, NGLYOXQ
      integer :: IWW
      real*8  :: QYIELD
      real*8 FWW,WWI,XX,BB

      ! min and max wavelength data available for BIACET
      real*8 :: GLYOX_MIN=250, GLYOX_MAX=460 ! markers for file length
      real*8 :: GLYOXQ_MIN =225
      ...
      IF (WW .lt. 250. .or. WW .gt. 460.) THEN
        ! zero q-yield outside of this range
        XNEW = 0.

       ELSE

       open (2, file='GLYOX/xglyox_in.dat', status='OLD')
         WB(:) = 0.d0
         FB(:) = 0.d0
         READ(2,'(i5)') NGLYOX
        ...
        open (3, file='GLYOX/qglyox_in.dat', status='OLD')
         WBQ(:) = 0.d0
         QY(:) = 0.d0
         READ(3,'(i5)') NGLYOXQ
...
c    Output, double precision YI(NI), the interpolated values.
        ...
        ! find nearest neighbour input WW to GLYOX wavelength
        call nearest_interp_1d ( NGLYOX, WB, WB, 1, INPWW, IND )

        ! quantum yield
        call nearest_interp_1d ( NGLYOXQ, WBQ, WBQ, 1, INPWW, INDQ )
        
        QYIELD = QY(INT( (INDQ(1) - GLYOXQ_MIN) / 5. ) + 1)
        XNEW = FB(INT(IND(1)) - INT(GLYOX_MIN) + 1) * 1e-20 * QYIELD

        ! if (INPWW(1) .gt. 225.) then
        !     if (first_call) then
        !     write(*,*) 'dip dupr', INPWW(1), INDQ(1), QYIELD
