...
c quantum-yield for BIACET is assumed 0.158 from 290 to 460 nm (Plum, 1983).
      ...
      real*8 :: BIACET_MIN=206, BIACET_MAX=492 ! markers for file length
      ...
      IF (WW .lt. 290. .or. WW .gt. 470.) THEN
        ! zero q-yield outside of this range
        XNEW = 0.

      ELSE

       open (2, file='BIACET/biacet_in.dat', status='OLD')
         WB(:) = 0.d0
         FB(:) = 0.d0
         READ(2,'(i5)') NBIACET
         READ(2,*)     QYIELD
...
c    Output, double precision YI(NI), the interpolated values.
        ...
        ! find nearest neighbour input WW to BIACET wavelength
         ...
         XNEW = FB(INT(IND(1)) - INT(BIACET_MIN) + 1) * 1e-20 * QYIELD
